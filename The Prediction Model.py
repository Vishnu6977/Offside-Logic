...
def create_rolling_features(matches: pd.DataFrame, feature_columns: list[str], rolling_window: int = 5) -> pd.DataFrame:
    ...
    # Concatenate all team rolling dataframes
    rolling_data = pd.concat(rolling_features, ignore_index=True)
    rolling_data = rolling_data.dropna(subset=feature_columns)  # Drop rows without enough history for rolling
...
def evaluate_model(combined: pd.DataFrame) -> tuple[float, float, float, float]:
    ...
    precision = precision_score(combined["actual"], combined["predicted"], average='micro')
    recall = recall_score(combined["actual"], combined["predicted"], average='micro')
    f1 = f1_score(combined["actual"], combined["predicted"], average='micro')
